...
export class UniverseChartComponent implements OnInit {
  constructor(private universeService: UniverseService, private store: FilterStateFacadeService) {}
  ...
  ngOnInit() {
    ...
    combineLatest([this._playersSubject.asObservable(), this.store.filterState$, this.resizeSubject])
    .pipe(
  ...
  updateChart(players: Player[], filters: FilterState) {
    players = players.filter((p) => applyFilters(p, filters)).sort((a, b) => b.points - a.points);
    ...
    .force('x', d3.forceX(bubbleChartWidth / 2).strength(0.02))
    .force('y', d3.forceY(bubbleChartHeight / 2).strength(0.02))
    .force(
      'anticollide',
    ...
    const bubbles = svg.selectAll('circle').data(players, (d: Player) => d.lastName);
    const texts = svg.selectAll('text').data(players, (d: Player) => d.lastName);
    ...
    const texts_enter = texts
      ...
      .on('click', (p) => this.selectPlayer(p, !this.selectionSticky || p !== this.selectedPlayer, true))
      .on('mouseover', (p) => this.selectPlayer(p, true, false))
      .on('mouseout', (p) => this.selectPlayer(p, false, false));
